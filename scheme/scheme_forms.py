...
def do_and_form(expressions, env):
    ...
    for item in expressions:
        if not is_scheme_true(scheme_eval(item, env)):
            return False
    return item
    # END PROBLEM 12
...
def do_or_form(expressions, env):
    ...
    # BEGIN PROBLEM 12
    for item in expressions:
        if is_scheme_true(scheme_eval(item, env)):
            return item
    return False
    # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    for binding in bindings:
        if not (scheme_listp(binding) and len(binding) == 2 and scheme_symbolp(binding.first)):
