...
def accuracy(typed, source):
    ...
    "*** YOUR CODE HERE ***"
    match = 0
    for i in len(source_words):
        if source_words[i] == typed_words[i]:
            match += 1
    return ('%{f.1}', match/len(source_words))
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    返回 WORD_LIST 中与 TYPED_WORD 基于 DIFF_FUNCTION 计算出的差异最小的元素。如果有多个单词的差异相同且最小，返回在 WORD_LIST 中位置最靠前的那个。如果差异大于 LIMIT，则返回 TYPED_WORD。
