...
def store_digits(n):
    ...
    if n < 10:
        return Link(n)
    s = store_digits(n // 10)   # 先把前面的都存好
    last = n % 10               # 最后一位（可能为 0）
    p = s
...
def two_list(vals, counts):
    ...
    Translation:返回一个链表，根据传入的两个列表。假设vals和counts大小相同。vals中的元素代表值，counts中对应的元素代表最终链表中所需的这种值的数量。假设counts中的所有元素都大于0。假设两个列表都有至少一个元素。
